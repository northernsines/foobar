...
class CCodeGenerator:
    ...
    def generate_console_class(self):
        ...
        self.indent()
        self.emit("int val = INT_MIN;  // Distinct error value")
        self.emit("if (scanf(\"%d\", &val) != 1) {")
        self.indent()
        self.emit("// Clear invalid input from buffer")
        self.emit("int c;")
        self.emit("while ((c = getchar()) != '\\n' && c != EOF);")
        self.emit("return INT_MIN;  // Return error sentinel")
        self.dedent()
        self.emit("}")
        self.emit("// Check if next character is a decimal point (reject floats)")
        self.emit("int next = getchar();")
        self.emit("if (next == '.') {")
        self.indent()
        self.emit("// This is a float, not an integer - reject it")
        self.emit("while ((next = getchar()) != '\\n' && next != EOF);")
        self.emit("return INT_MIN;  // Return error sentinel")
        self.dedent()
        self.emit("}")
        self.emit("// Put back the character if it wasn't a decimal (likely newline)")
        self.emit("if (next != '\\n' && next != EOF) {")
        self.indent()
        self.emit("ungetc(next, stdin);")
        self.dedent()
        self.emit("}")
        ...
        self.indent()
        self.emit("float val = NAN;  // Distinct error value")
        self.emit("if (scanf(\"%f\", &val) != 1) {")
        self.indent()
        self.emit("// Clear invalid input from buffer")
        self.emit("int c;")
        self.emit("while ((c = getchar()) != '\\n' && c != EOF);")
        self.emit("return NAN;  // Return error sentinel")
        self.dedent()
        self.emit("}")
        self.emit("getchar(); // Consume newline after valid input")
        self.emit("return val;")
        self.dedent()
        self.emit("}")
        self.emit("")
        ...
        self.emit("// Accept: true, false, 1, 0 (case-insensitive for true/false)")
        self.emit("if (strcmp(input, \"true\") == 0 || strcmp(input, \"True\") == 0 || strcmp(input, \"TRUE\") == 0 || strcmp(input, \"1\") == 0) {")
        self.indent()
        self.emit("return true;")
        self.dedent()
        self.emit("} else if (strcmp(input, \"false\") == 0 || strcmp(input, \"False\") == 0 || strcmp(input, \"FALSE\") == 0 || strcmp(input, \"0\") == 0) {")
        self.indent()
        self.emit("return false;")
        self.dedent()
        self.emit("} else {")
        self.indent()
        self.emit("// Invalid input - return false but print warning")
        self.emit("fprintf(stderr, \"Warning: Invalid boolean input '%s', treating as false\\n\", input);")
        self.emit("return false;")
        self.dedent()
        self.emit("}")
        self.dedent()
