...
class CCodeGenerator:
    ...
    def generate(self, program: Program) -> str:
        ...
        for class_name, cls in self.classes.items():
            # Find Initialize parameters
            init_params = ""
            init_method = None
            for member in cls.members:
                if isinstance(member, MethodDecl) and member.name == "Initialize":
                    params = ', '.join([f"{self.c_type(p.param_type)} {p.name}" for p in member.parameters])
                    init_params = params
                    init_method = member
                    break
            ...
            if init_params:
                self.emit(f"{class_name}* {class_name}_new({init_params});")
            else:
                self.emit(f"{class_name}* {class_name}_new(void);")
            
            # Forward declare all methods (including Initialize)
            for member in cls.members:
                if isinstance(member, MethodDecl):
                    ...
                    self.emit(f"{return_type} {class_name}_{member.name}({params_str});")
                    ...
                    for parent_member in parent_cls.members:
                        if isinstance(parent_member, MethodDecl):
                            ...
                            if not overridden and parent_member.name != "Initialize":
                                ...
                                params_str = ', '.join(params)
                                self.emit(f"{return_type} {class_name}_{parent_member.name}({params_str});")
    ...
    def generate_class_constructor(self, cls: ClassDecl):
        ...
        # Find the Initialize method if it exists
        init_method = None
        for member in cls.members:
            if isinstance(member, MethodDecl) and member.name == "Initialize":
                init_method = member
        ...
        if init_method:
            # Generate constructor with Initialize logic
            params = ', '.join([f"{self.c_type(p.param_type)} {p.name}" for p in init_method.parameters])
        else:
            # Generate default constructor (no parameters)
            params = ""
        
        if params:
            self.emit(f"{cls.name}* {cls.name}_new({params}) {{")
        else:
            self.emit(f"{cls.name}* {cls.name}_new(void) {{")
        self.indent()
        ...
        if init_method:
            param_names = ', '.join([p.name for p in init_method.parameters])
            self.emit(f"{cls.name}_Initialize(obj, {param_names});")
    ...
    def generate_inherited_method_wrapper(self, class_name: str, parent_name: str, method: MethodDecl):
        ...
        self.emit(f"{return_type} {class_name}_{method.name}({params_str}) {{")
        self.indent()
        ...
        if param_names:
            if return_type != "void":
                self.emit(f"return {parent_name}_{method.name}(({parent_name}*)thisclass, {param_names});")
            else:
                self.emit(f"{parent_name}_{method.name}(({parent_name}*)thisclass, {param_names});")
        else:
            if return_type != "void":
                self.emit(f"return {parent_name}_{method.name}(({parent_name}*)thisclass);")
            else:
                self.emit(f"{parent_name}_{method.name}(({parent_name}*)thisclass);")
        ...
        if isinstance(expr, Literal):
            ...
            if expr.object:
                ...
                if is_parent_call:
                    ...
                    if self.current_class and self.current_class in self.classes:
                        ...
                        if cls.parent_classes:
                            ...
                            return f"{parent_class}_{expr.method_name}({args_str})"
                ...
                if obj_type and obj_type in self.classes:
                    ...
                    args_str = ', '.join(args_list)
                    return f"{obj_type}_{expr.method_name}({args_str})"
                else:
